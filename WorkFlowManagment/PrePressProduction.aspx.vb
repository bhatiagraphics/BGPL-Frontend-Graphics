...
Partial Class PrePressProduction
    ...
    Private Sub Fillcombo()
        Try
            ...
            dt3 = objDas.GetDataTable(sSQL)
            dt3.Rows.InsertAt(dt3.NewRow, 0)
            ddlassignedto_prepress.DataSource = dt3
            ddlassignedto_prepress.DataTextField = "u_name"
            ddlassignedto_prepress.DataValueField = "u_id"
            ddlassignedto_prepress.DataBind()
    ...
    Protected Sub New_Docno()
        Try
            j = clsJobSendToPrePressProduction.Fetch_Paramno("")
            txtjobid.Text = clsJobSendToPrePressProduction.Generate_Docno(j, "", "").ToString.Trim
        Catch ex As Exception
            ErrorMSG.Visible = True
            ErrorMSG.Text = ""
            ErrorMSG.Text = ex.Message
    ...
    Private Sub DisplayData(ByVal geno As String)
        ...
        con = clsJobSendToPrePressProduction.GetConnection()
        com = clsJobSendToPrePressProduction.GetCommand
        com.Connection = con
        com.CommandType = CommandType.Text
        Try
            Dim obj As clsJobSendToPrePressProduction = clsJobSendToPrePressProduction.FillData(geno, "B")
            If Not IsNothing(obj) Then
                ...
                ddlassignedto_prepress.SelectedItem.Selected = False
                ddlassignedto_prepress.Items.FindByValue(Trim(obj.assignedto_prepress)).Selected = True
                ...
                ddlread_unread.SelectedItem.Selected = False
                ddlread_unread.Items.FindByValue(Trim(obj.read_unread)).Selected = True

                txtsendfor_reproduction_remark.Text = Trim(obj.sendfor_reproduction_remark)
                txtcustomer_approval_remark.Text = Trim(obj.customer_approval_remark)
                ...
                If Trim(obj.sendfor_reproduction_flag) = "1" Then
                    btnSendToReproduction.Visible = False
                    btnSendToCustomerApproval.Visible = False
                    btnSave.Visible = False
                    btnSave.Visible = False
    ...
    Private Sub Save_Data_Add()
        Dim con As SqlConnection
        Dim com As SqlCommand
        Dim c As New clsJobSendToPrePressProduction
        Dim i As Integer
        ...
        Try
            Dim obj As New clsJobSendToPrePressProduction, objDas As New DBAccess
            ...
            obj.assignedto_prepress = ddlassignedto_prepress.SelectedValue
            ...
            clsJobSendToPrePressProduction.Update_Data_Add(obj, j, mode, "")
    ...
    Private Sub Save_Data_Edit()
        ...
        Try
            ...
            obj.printercd = ddlprintercd.SelectedValue
            obj.read_unread = ddlread_unread.SelectedValue
            ...
            clsJobSendToPrePressProduction.Update_Data_Edit(obj, j, mode, "")
    ...
    Private Sub save_edit()
        ...
        If ErrorMSG.Text = "" Then
            ...
            Response.Write("window.location='PrePressProductionSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
        End If
    End Sub
    ...
    Protected Sub btnHold_Click(sender As Object, e As System.EventArgs) Handles btnhold.Click
        Try
            ...
            Dim dt1, dt2, dt3 As DataTable
            Dim objDas As New DBAccess
            sSQL = "update jobentry set hold_sendforprepress_flag='1',approve_sendforprepress_flag='',approve_sendforprepress_uid='',approve_sendforprepress_date=null where jobid='" & txtjobid.Text & "'"
            dt1 = objDas.GetDataTable(sSQL)
    ...
    Protected Sub btnSendToGrphics_Click(sender As Object, e As System.EventArgs) Handles btnSendToReproduction.Click
        Try
            Call Save_Data_Edit()
            Dim sSQL As String
            Dim dt1, dt2, dt3 As DataTable
            Dim objDas As New DBAccess
            If Trim(ddlprintcd.SelectedValue) = GetPrintingType() Then
                sSQL = "update jobentry set sendfor_reproduction_flag='1',sendfor_reproduction_uid='" & Trim(Session.Item("UserID")) & "',sendfor_reproduction_udate=GetDate(),sendfor_challan_flag='1',sendfor_challan_uid='" & Trim(Session.Item("UserID")) & "',sendfor_challan_date=GetDate(),assignedto_prepress='" & Trim(ddlassignedto_prepress.SelectedValue) & "',assignedto_challan='" & Trim(ddlassignedto_prepress.SelectedValue) & "',assignedto_dispatch='" & Trim(ddlassignedto_prepress.SelectedValue) & "',assignedto_billing='" & Trim(ddlassignedto_prepress.SelectedValue) & "'  where jobid='" & txtjobid.Text & "'"
                dt1 = objDas.GetDataTable(sSQL)
                Response.Write("<script>alert('Send To Challan Succesfully'); ")
                Response.Write("window.location='PrePressProductionSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
            Else
                sSQL = "update jobentry set sendfor_reproduction_flag='1',sendfor_reproduction_uid='" & Trim(Session.Item("UserID")) & "',sendfor_reproduction_udate=GetDate(),sendfor_reproduction_remark='" & txtsendfor_reproduction_remark.Text & "',assignedto_prepress='" & Trim(ddlassignedto_prepress.SelectedValue) & "',assignedto_materialprod='" & Trim(ddlassignedto_prepress.SelectedValue) & "',assignedto_qccheck='" & Trim(ddlassignedto_prepress.SelectedValue) & "',assignedto_challan='" & Trim(ddlassignedto_prepress.SelectedValue) & "',assignedto_dispatch='" & Trim(ddlassignedto_prepress.SelectedValue) & "',assignedto_billing='" & Trim(ddlassignedto_prepress.SelectedValue) & "'  where jobid='" & txtjobid.Text & "'"
                dt1 = objDas.GetDataTable(sSQL)
                Response.Write("<script>alert('Send To Reproduction Succesfully'); ")
                Response.Write("window.location='PrePressProductionSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
            End If
        Catch ex As Exception
            ErrorMSG.Visible = True
    ...
    Protected Sub btnSendToCustomerApproval_Click(sender As Object, e As System.EventArgs) Handles btnSendToCustomerApproval.Click
        Try
            Call Save_Data_Edit()
            Dim sSQL As String
            Dim dt1, dt2, dt3 As DataTable
            Dim objDas As New DBAccess
            sSQL = "update jobentry set sento_customer_flag='1',approve_sendtocustomerforapprval_flag='0',approve_sendforprepress_flag='0',sento_customer_uid='" & Trim(Session.Item("UserID")) & "',sento_customer_date=GetDate(),customer_approval_remark='" & txtcustomer_approval_remark.Text & "' where jobid='" & txtjobid.Text & "'"
            dt1 = objDas.GetDataTable(sSQL)
            Response.Write("<script>alert('Send To Customer Approval Succesfully'); ")
            Response.Write("window.location='PrePressProductionSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
    ...
    Protected Sub btnClose_Click(sender As Object, e As System.EventArgs) Handles btnClose.Click
        Response.Redirect("PrePressProductionSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "")
    End Sub
    ...
    Private Function GetPrintingType() As String
        GetPrintingType = ""
        Dim sSQL As String, dt1 As DataTable, objDas As New DBAccess
        sSQL = "Select printingtype From Fasetup "
        dt1 = objDas.GetDataTable(sSQL)
        If dt1.Rows.Count > 0 Then
            GetPrintingType = Trim(dt1.Rows(0)("printingtype").ToString)
        Else
            GetPrintingType = ""
        End If
    End Function
