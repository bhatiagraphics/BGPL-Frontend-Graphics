...
Partial Class ChallanFormSearch
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_jobChallan_GetData '" & txtJobId.Text & "','" & txtJobName.Text & "','" & txtInternalCode.Text & "','" & ddlprioirty.SelectedValue & "','" & ddlassignedto.SelectedValue & "','" & ddlcuscode.SelectedValue & "','" & txtjobcreatedt.Text & "','" & txtticketno.Text & "','A','" & Trim(Session.Item("UserID")) & "','" & ddlStatus.SelectedValue & "' "
    ...
    Protected Sub btnEditEntry_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If Trim(roomName) <> "" Then
            ...
            ScriptManager.RegisterStartupScript(Page, GetType(Page), "OpenWindow", "window.open('ChallanForm.aspx?docno=" & roomName & "&MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "');", True)
