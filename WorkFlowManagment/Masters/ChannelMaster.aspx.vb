...
Partial Class ChannelMaster
    ...
    Private Sub DisplayData(ByVal channelcode As String)
        Try
            Dim obj As ClsChannelMaster = ClsChannelMaster.FillData(channelcode, "B")
            If Not IsNothing(obj) Then
                HFChannelCode.Value = obj.channelcode
                txtChannelName.Text = Trim(obj.channelname)
...
#End Region
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_ChannelMaster_GetData '','A' "
    ...
    Protected Sub btnEditEntry_Click(sender As Object, e As System.EventArgs)
        ...
        Dim roomName As String = DataBinder.Eval(container.DataItem, "channelcode").ToString()
    ...
    Public Function Check_Duplicate() As Boolean
        Try
            ...
            strcode = ClsChannelMaster.CheckDuplicate(txtChannelName.Text).ToString
            If Trim(strcode) <> Trim(HFChannelCode.Value) And strcode <> "" Then
                Return True
            Else
                Return False
                Exit Function
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim obj As New ClsChannelMaster, objDas As New DBAccess
            obj.channelcode = HFChannelCode.Value
            obj.channelname = objDas.FilteredString(txtChannelName.Text)

            If HFChannelCode.Value = "" Then
                obj.channelcode = ClsChannelMaster.GetDocCode
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtChannelName.Text & "' Already Exist"
                    ErrorMsg0.Visible = True
                    Exit Try
                Else
                    ClsChannelMaster.Update_Data(obj, "ADD")
                End If
            Else
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtChannelName.Text & "' Already Exist"
                    ...
                    Exit Try
                Else
                    ClsChannelMaster.Update_Data(obj, "EDIT")
                End If
            End If
            'FillData()
            CreateGridView()
    ...
    Protected Sub btnDelete_Click(sender As Object, e As System.EventArgs) Handles btnDelete.Click
        Try
            If HFChannelCode.Value <> "" Then
                Dim str As String
                str = ClsChannelMaster.Check_Integrity(HFChannelCode.Value).ToString.Trim
                If str <> "" Then
                    ...
                    Dim myscript As String = "alert('Channel Being Used In " & Trim(str) & "');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)

                    ErrorMsg0.Visible = True
                    ErrorMsg0.Text = ""
                    ErrorMsg0.Text = "Channel Being Used In " & Trim(str)
                    str = ""
                    Exit Sub
                End If

                Dim obj As New ClsChannelMaster
                obj.channelcode = HFChannelCode.Value
                ClsChannelMaster.Delete(obj)
                'FillData()
                CreateGridView()
                showPopUp(False)
            End If
