...
Partial Class DesignationMaster
    ...
    Private Sub Refresh()
        HFDesignationCode.Value = ""
        txtDesignationName.Text = ""
        ErrorMsg.Text = ""
        ErrorMsg0.Text = ""
    End Sub

    Private Sub DisplayData(ByVal desigcd As String)
        Try
            Dim obj As ClsDesignationMaster = ClsDesignationMaster.FillData(desigcd, "B")
            If Not IsNothing(obj) Then
                HFDesignationCode.Value = obj.desigcd
                txtDesignationName.Text = Trim(obj.designame)
...
#End Region
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_designation_GetData '','A' "
    ...
    Protected Sub btnEditEntry_Click(sender As Object, e As System.EventArgs)
        ...
        Dim roomName As String = DataBinder.Eval(container.DataItem, "desigcd").ToString()
    ...
    Public Function Check_Duplicate() As Boolean
        Try
            ...
            strcode = ClsDesignationMaster.CheckDuplicate(txtDesignationName.Text).ToString
            If Trim(strcode) <> Trim(HFDesignationCode.Value) And strcode <> "" Then
                Return True
            Else
                Return False
                Exit Function
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim obj As New ClsDesignationMaster, objDas As New DBAccess
            obj.desigcd = HFDesignationCode.Value
            obj.designame = objDas.FilteredString(txtDesignationName.Text)

            If HFDesignationCode.Value = "" Then
                obj.desigcd = ClsDesignationMaster.GetDocCode
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtDesignationName.Text & "' Already Exist"
                    ErrorMsg0.Visible = True
                    Exit Try
                Else
                    ClsDesignationMaster.Update_Data(obj, "ADD")
                End If
            Else
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtDesignationName.Text & "' Already Exist"
                    ...
                    Exit Try
                Else
                    ClsDesignationMaster.Update_Data(obj, "EDIT")
                End If
            End If
            'FillData()
            CreateGridView()
    ...
    Protected Sub btnDelete_Click(sender As Object, e As System.EventArgs) Handles btnDelete.Click
        Try
            If HFDesignationCode.Value <> "" Then
                Dim str As String
                str = ClsDesignationMaster.Check_Integrity(HFDesignationCode.Value).ToString.Trim
                If str <> "" Then
                    ...
                    Dim myscript As String = "alert('Designation Being Used In " & Trim(str) & "');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)

                    ErrorMsg0.Visible = True
                    ErrorMsg0.Text = ""
                    ErrorMsg0.Text = "Designation Being Used In " & Trim(str)
                    str = ""
                    Exit Sub
                End If
                ...
                obj.desigcd = HFDesignationCode.Value
                ClsDesignationMaster.Delete(obj)
                'FillData()
                CreateGridView()
                showPopUp(False)
            End If
