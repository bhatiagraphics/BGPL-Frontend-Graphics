...
Partial Class OperatorMaster
    ...
    Private Sub DisplayData(ByVal opcode As String)
        Try
            Dim obj As ClsOperatorMaster = ClsOperatorMaster.FillData(opcode, "B")
            If Not IsNothing(obj) Then
                HFopcode.Value = obj.opcode
                txtopname.Text = Trim(obj.opname)
...
#End Region
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_operator_GetData '','A' "
    ...
    Protected Sub btnEditEntry_Click(sender As Object, e As System.EventArgs)
        ...
        Dim roomName As String = DataBinder.Eval(container.DataItem, "opcode").ToString()
    ...
    Public Function Check_Duplicate() As Boolean
        Try
            ...
            strcode = ClsOperatorMaster.CheckDuplicate(txtopname.Text).ToString
            If Trim(strcode) <> Trim(HFopcode.Value) And strcode <> "" Then
                Return True
            Else
                Return False
                Exit Function
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim obj As New ClsOperatorMaster, objDas As New DBAccess
            obj.opcode = HFopcode.Value
            obj.opname = objDas.FilteredString(txtopname.Text)

            If HFopcode.Value = "" Then
                obj.opcode = ClsOperatorMaster.GetDocCode
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtopname.Text & "' Already Exist"
                    ErrorMsg0.Visible = True
                    Exit Try
                Else
                    ClsOperatorMaster.Update_Data(obj, "ADD")
                End If
            Else
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtopname.Text & "' Already Exist"
                    ...
                    Exit Try
                Else
                    ClsOperatorMaster.Update_Data(obj, "EDIT")
                End If
            End If
            'FillData()
            CreateGridView()
    ...
    Protected Sub btnDelete_Click(sender As Object, e As System.EventArgs) Handles btnDelete.Click
        Try
            If HFopcode.Value <> "" Then
                Dim str As String
                str = ClsOperatorMaster.Check_Integrity(HFopcode.Value).ToString.Trim
                If str <> "" Then
                    ...
                    Dim myscript As String = "alert('operator Being Used In " & Trim(str) & "');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)

                    ErrorMsg0.Visible = True
                    ErrorMsg0.Text = ""
                    ErrorMsg0.Text = "operator Being Used In " & Trim(str)
                    str = ""
                    Exit Sub
                End If
