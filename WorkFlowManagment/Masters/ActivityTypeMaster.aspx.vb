...
Partial Class ActivityTypeMaster
    ...
    Private Sub DisplayData(ByVal acttypecd As String)
        Try
            Dim obj As ClsActivityTypeMaster = ClsActivityTypeMaster.FillData(acttypecd, "B")
            If Not IsNothing(obj) Then
                HFacttypecd.Value = obj.acttypecd
                txtsrno.Text = Trim(obj.srno)

                txtacttypename.Text = Trim(obj.acttypename)
...
#End Region
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_ActivityTypeMaster_GetData '','A' "
    ...
    Protected Sub btnEditEntry_Click(sender As Object, e As System.EventArgs)
        ...
        Dim roomName As String = DataBinder.Eval(container.DataItem, "acttypecd").ToString()
    ...
    Public Function Check_Duplicate() As Boolean
        Try
            ...
            strcode = ClsActivityTypeMaster.CheckDuplicate(txtacttypename.Text).ToString
            If Trim(strcode) <> Trim(HFacttypecd.Value) And strcode <> "" Then
                Return True
            Else
                Return False
                Exit Function
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim obj As New ClsActivityTypeMaster, objDas As New DBAccess
            obj.acttypecd = HFacttypecd.Value
            obj.srno = objDas.FilteredString(txtsrno.Text)

            obj.acttypename = objDas.FilteredString(txtacttypename.Text)

            If HFacttypecd.Value = "" Then
                obj.acttypecd = ClsActivityTypeMaster.GetDocCode
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtacttypename.Text & "' Already Exist"
                    ErrorMsg0.Visible = True
                    Exit Try
                Else
                    ClsActivityTypeMaster.Update_Data(obj, "ADD")
                End If
            Else
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtacttypename.Text & "' Already Exist"
                    ...
                    Exit Try
                Else
                    ClsActivityTypeMaster.Update_Data(obj, "EDIT")
                End If
            End If
            'FillData()
            CreateGridView()
    ...
    Protected Sub btnDelete_Click(sender As Object, e As System.EventArgs) Handles btnDelete.Click
        Try
            If HFacttypecd.Value <> "" Then
                Dim str As String
                str = ClsActivityTypeMaster.Check_Integrity(HFacttypecd.Value).ToString.Trim
                If str <> "" Then
                    ...
                    Dim myscript As String = "alert('Activity Type Being Used In " & Trim(str) & "');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)

                    ErrorMsg0.Visible = True
                    ErrorMsg0.Text = ""
                    ErrorMsg0.Text = "Activity Type Being Used In " & Trim(str)
                    str = ""
                    Exit Sub
                End If

                Dim obj As New ClsActivityTypeMaster
                obj.acttypecd = HFacttypecd.Value
                ClsActivityTypeMaster.Delete(obj)
                'FillData()
                CreateGridView()
                showPopUp(False)
            End If
