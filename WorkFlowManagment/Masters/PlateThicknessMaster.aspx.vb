...
Partial Class PlateThicknessMaster
    ...
    Private Sub DisplayData(ByVal thickcd As String)
        Try
            Dim obj As ClsPlateThicknessMaster = ClsPlateThicknessMaster.FillData(thickcd, "B")
            If Not IsNothing(obj) Then
                HFthickcd.Value = obj.thickcd
                txtPlateThickness.Text = Trim(obj.thickness)
...
#End Region
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_platethickness_GetData '','A' "
    ...
    Protected Sub btnEditEntry_Click(sender As Object, e As System.EventArgs)
        ...
        Dim roomName As String = DataBinder.Eval(container.DataItem, "thickcd").ToString()
    ...
    Public Function Check_Duplicate() As Boolean
        Try
            ...
            strcode = ClsPlateThicknessMaster.CheckDuplicate(txtPlateThickness.Text).ToString
            If Trim(strcode) <> Trim(HFthickcd.Value) And strcode <> "" Then
                Return True
            Else
                Return False
                Exit Function
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            If txtPlateThickness.Text = "" Then
                ErrorMsg0.Text = ""
            ...
            Dim obj As New ClsPlateThicknessMaster, objDas As New DBAccess
            obj.thickcd = HFthickcd.Value
            obj.thickness = objDas.FilteredString(txtPlateThickness.Text)

            If HFthickcd.Value = "" Then
                obj.thickcd = ClsPlateThicknessMaster.GetDocCode
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtPlateThickness.Text & "' Already Exist"
                    ErrorMsg0.Visible = True
                    Exit Try
                Else
                    ClsPlateThicknessMaster.Update_Data(obj, "ADD")
                End If
            Else
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtPlateThickness.Text & "' Already Exist"
                    ...
                    Exit Try
                Else
                    ClsPlateThicknessMaster.Update_Data(obj, "EDIT")
                End If
            End If
            'FillData()
            CreateGridView()
    ...
    Protected Sub btnDelete_Click(sender As Object, e As System.EventArgs) Handles btnDelete.Click
        Try
            If HFthickcd.Value <> "" Then
                Dim str As String
                str = ClsPlateThicknessMaster.Check_Integrity(HFthickcd.Value).ToString.Trim
                If str <> "" Then
                    ...
                    Dim myscript As String = "alert('Plate Thickness Being Used In " & Trim(str) & "');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)

                    ErrorMsg0.Visible = True
                    ErrorMsg0.Text = ""
                    ErrorMsg0.Text = "Plate Thickness Being Used In " & Trim(str)
                    str = ""
                    Exit Sub
                End If

                Dim obj As New ClsPlateThicknessMaster
                obj.thickcd = HFthickcd.Value
                ClsPlateThicknessMaster.Delete(obj)
                'FillData()
                CreateGridView()
                showPopUp(False)
            End If
