...
Partial Class PrintingMaster
    ...
    Private Sub Refresh()
        HFPrintingCode.Value = ""
        txtPrintingName.Text = ""
        ErrorMsg.Text = ""
        ErrorMsg0.Text = ""
    End Sub

    Private Sub DisplayData(ByVal printcd As String)
        Try
            Dim obj As ClsPrintingMaster = ClsPrintingMaster.FillData(printcd, "B")
            If Not IsNothing(obj) Then
                HFPrintingCode.Value = obj.printcd
                txtPrintingName.Text = Trim(obj.printname)
...
#End Region
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_printing_GetData '','A' "
    ...
    Protected Sub btnEditEntry_Click(sender As Object, e As System.EventArgs)
        ...
        Dim roomName As String = DataBinder.Eval(container.DataItem, "printcd").ToString()
    ...
    Public Function Check_Duplicate() As Boolean
        Try
            ...
            strcode = ClsPrintingMaster.CheckDuplicate(txtPrintingName.Text).ToString
            If Trim(strcode) <> Trim(HFPrintingCode.Value) And strcode <> "" Then
                Return True
            Else
                Return False
                Exit Function
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim obj As New ClsPrintingMaster, objDas As New DBAccess
            obj.printcd = HFPrintingCode.Value
            obj.printname = objDas.FilteredString(txtPrintingName.Text)

            If HFPrintingCode.Value = "" Then
                obj.printcd = ClsPrintingMaster.GetDocCode
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtPrintingName.Text & "' Already Exist"
                    ErrorMsg0.Visible = True
                    Exit Try
                Else
                    ClsPrintingMaster.Update_Data(obj, "ADD")
                End If
            Else
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtPrintingName.Text & "' Already Exist"
                    ...
                    Exit Try
                Else
                    ClsPrintingMaster.Update_Data(obj, "EDIT")
                End If
            End If
            'FillData()
            CreateGridView()
    ...
    Protected Sub btnDelete_Click(sender As Object, e As System.EventArgs) Handles btnDelete.Click
        Try
            If HFPrintingCode.Value <> "" Then
                Dim str As String
                str = ClsPrintingMaster.Check_Integrity(HFPrintingCode.Value).ToString.Trim
                If str <> "" Then
                    ...
                    Dim myscript As String = "alert('Printing Being Used In " & Trim(str) & "');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)

                    ErrorMsg0.Visible = True
                    ErrorMsg0.Text = ""
                    ErrorMsg0.Text = "Printing Being Used In " & Trim(str)
                    str = ""
                    Exit Sub
                End If

                Dim obj As New ClsPrintingMaster
                obj.printcd = HFPrintingCode.Value
                ClsPrintingMaster.Delete(obj)
                'FillData()
                CreateGridView()
                showPopUp(False)
            End If
