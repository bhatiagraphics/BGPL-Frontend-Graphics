...
Partial Class TransportMaster
    ...
    Private Sub DisplayData(ByVal transcd As String)
        Try
            Dim obj As ClsTransportMaster = ClsTransportMaster.FillData(transcd, "B")
            If Not IsNothing(obj) Then
                HFtranscd.Value = obj.transcd
                txttransname.Text = Trim(obj.transname)
...
#End Region
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_transport_GetData '','A' "
    ...
    Protected Sub btnEditEntry_Click(sender As Object, e As System.EventArgs)
        ...
        Dim roomName As String = DataBinder.Eval(container.DataItem, "transcd").ToString()
    ...
    Public Function Check_Duplicate() As Boolean
        Try
            ...
            strcode = ClsTransportMaster.CheckDuplicate(txttransname.Text).ToString
            If Trim(strcode) <> Trim(HFtranscd.Value) And strcode <> "" Then
                Return True
            Else
                Return False
                Exit Function
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim obj As New ClsTransportMaster, objDas As New DBAccess
            obj.transcd = HFtranscd.Value
            obj.transname = objDas.FilteredString(txttransname.Text)

            If HFtranscd.Value = "" Then
                obj.transcd = ClsTransportMaster.GetDocCode
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txttransname.Text & "' Already Exist"
                    ErrorMsg0.Visible = True
                    Exit Try
                Else
                    ClsTransportMaster.Update_Data(obj, "ADD")
                End If
            Else
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txttransname.Text & "' Already Exist"
                    ...
                    Exit Try
                Else
                    ClsTransportMaster.Update_Data(obj, "EDIT")
                End If
            End If
            'FillData()
            CreateGridView()
    ...
    Protected Sub btnDelete_Click(sender As Object, e As System.EventArgs) Handles btnDelete.Click
        Try
            If HFtranscd.Value <> "" Then
                Dim str As String
                str = ClsTransportMaster.Check_Integrity(HFtranscd.Value).ToString.Trim
                If str <> "" Then
                    ...
                    Dim myscript As String = "alert('Transport Being Used In " & Trim(str) & "');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)

                    ErrorMsg0.Visible = True
                    ErrorMsg0.Text = ""
                    ErrorMsg0.Text = "Transport Being Used In " & Trim(str)
                    str = ""
                    Exit Sub
                End If

                Dim obj As New ClsTransportMaster
                obj.transcd = HFtranscd.Value
                ClsTransportMaster.Delete(obj)
                'FillData()
                CreateGridView()
                showPopUp(False)
            End If
