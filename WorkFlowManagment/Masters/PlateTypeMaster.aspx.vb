...
Partial Class PlateTypeMaster
    ...
    Private Sub DisplayData(ByVal platetypecd As String)
        Try
            Dim obj As ClsPlateTypeMaster = ClsPlateTypeMaster.FillData(platetypecd, "B")
            If Not IsNothing(obj) Then
                HFplatetypcd.Value = obj.platetypecd
                txtplatetypename.Text = Trim(obj.platetypename)
...
#End Region
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_platetype_GetData '','A' "
    ...
    Protected Sub btnEditEntry_Click(sender As Object, e As System.EventArgs)
        ...
        Dim roomName As String = DataBinder.Eval(container.DataItem, "platetypecd").ToString()
    ...
    Public Function Check_Duplicate() As Boolean
        Try
            ...
            strcode = ClsPlateTypeMaster.CheckDuplicate(txtplatetypename.Text).ToString
            If Trim(strcode) <> Trim(HFplatetypcd.Value) And strcode <> "" Then
                Return True
            Else
                Return False
                Exit Function
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim obj As New ClsPlateTypeMaster, objDas As New DBAccess
            obj.platetypecd = HFplatetypcd.Value
            obj.platetypename = objDas.FilteredString(txtplatetypename.Text)

            If HFplatetypcd.Value = "" Then
                obj.platetypecd = ClsPlateTypeMaster.GetDocCode
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtplatetypename.Text & "' Already Exist"
                    ErrorMsg0.Visible = True
                    Exit Try
                Else
                    ClsPlateTypeMaster.Update_Data(obj, "ADD")
                End If
            Else
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtplatetypename.Text & "' Already Exist"
                    ...
                    Exit Try
                Else
                    ClsPlateTypeMaster.Update_Data(obj, "EDIT")
                End If
            End If
            'FillData()
            CreateGridView()
    ...
    Protected Sub btnDelete_Click(sender As Object, e As System.EventArgs) Handles btnDelete.Click
        Try
            If HFplatetypcd.Value <> "" Then
                Dim str As String
                str = ClsPlateTypeMaster.Check_Integrity(HFplatetypcd.Value).ToString.Trim
                If str <> "" Then
                    ...
                    Dim myscript As String = "alert('Plate Type Being Used In " & Trim(str) & "');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)

                    ErrorMsg0.Visible = True
                    ErrorMsg0.Text = ""
                    ErrorMsg0.Text = "Plate Type Being Used In " & Trim(str)
                    str = ""
                    Exit Sub
                End If

                Dim obj As New ClsPlateTypeMaster
                obj.platetypecd = HFplatetypcd.Value
                ClsPlateTypeMaster.Delete(obj)
                'FillData()
                CreateGridView()
                showPopUp(False)
            End If
