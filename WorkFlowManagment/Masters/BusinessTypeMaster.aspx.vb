...
Partial Class BusinessTypeMaster 
    ...
    Private Sub DisplayData(ByVal bstypecd As String)
        Try
            Dim obj As ClsBusinessTypeMaster = ClsBusinessTypeMaster.FillData(bstypecd, "B")
            If Not IsNothing(obj) Then
                HFbstypecd.Value = obj.bstypecd
                txtbstypename.Text = Trim(obj.bstypename)
...
#End Region
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_BusinessTypeMaster_GetData '','A' "
    ...
    Private Sub Set_UserAccessRights()
        ...
        c = DBAccess.Accessright(Trim(Session.Item("UserID")), Trim("142"))
    ...
    Protected Sub btnEditEntry_Click(sender As Object, e As System.EventArgs)
        ...
        Dim roomName As String = DataBinder.Eval(container.DataItem, "bstypecd").ToString()
    ...
    Public Function Check_Duplicate() As Boolean
        Try
            ...
            strcode = ClsBusinessTypeMaster.CheckDuplicate(txtbstypename.Text).ToString
            If Trim(strcode) <> Trim(HFbstypecd.Value) And strcode <> "" Then
                Return True
            Else
                Return False
                Exit Function
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim obj As New ClsBusinessTypeMaster, objDas As New DBAccess
            obj.bstypecd = HFbstypecd.Value
            obj.bstypename = objDas.FilteredString(txtbstypename.Text)

            If HFbstypecd.Value = "" Then
                obj.bstypecd = ClsBusinessTypeMaster.GetDocCode
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtbstypename.Text & "' Already Exist"
                    ErrorMsg0.Visible = True
                    Exit Try
                Else
                    ClsBusinessTypeMaster.Update_Data(obj, "ADD")
                End If
            Else
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtbstypename.Text & "' Already Exist"
                    ...
                    Exit Try
                Else
                    ClsBusinessTypeMaster.Update_Data(obj, "EDIT")
                End If
            End If
            'FillData()
            CreateGridView()
    ...
    Protected Sub btnDelete_Click(sender As Object, e As System.EventArgs) Handles btnDelete.Click
        Try
            If HFbstypecd.Value <> "" Then
                Dim str As String
                str = ClsBusinessTypeMaster.Check_Integrity(HFbstypecd.Value).ToString.Trim
                If str <> "" Then
                    ...
                    Dim myscript As String = "alert('Business Type Being Used In " & Trim(str) & "');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)

                    ErrorMsg0.Visible = True
                    ErrorMsg0.Text = ""
                    ErrorMsg0.Text = "Business Type Being Used In " & Trim(str)
                    str = ""
                    Exit Sub
                End If

                Dim obj As New ClsBusinessTypeMaster
                obj.bstypecd = HFbstypecd.Value
                ClsBusinessTypeMaster.Delete(obj)
                'FillData()
                CreateGridView()
                showPopUp(False)
            End If
