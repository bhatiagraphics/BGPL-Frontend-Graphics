...
Partial Class LeadTypeMaster
    ...
    Private Sub Refresh()
        HFleadtypecd.Value = ""
        txtleadtypename.Text = ""
        ErrorMsg.Text = ""
        ErrorMsg0.Text = ""
    End Sub

    Private Sub DisplayData(ByVal leadtypecd As String)
        Try
            Dim obj As ClsLeadTypeMaster = ClsLeadTypeMaster.FillData(leadtypecd, "B")
            If Not IsNothing(obj) Then
                HFleadtypecd.Value = obj.leadtypecd
                txtleadtypename.Text = Trim(obj.leadtypename)
...
#End Region
    ...
    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Sp_LeadTypeMaster_GetData '','A' "
    ...
    Protected Sub btnEditEntry_Click(sender As Object, e As System.EventArgs)
        ...
        Dim roomName As String = DataBinder.Eval(container.DataItem, "leadtypecd").ToString()
    ...
    Public Function Check_Duplicate() As Boolean
        Try
            ...
            strcode = ClsLeadTypeMaster.CheckDuplicate(txtleadtypename.Text).ToString
            If Trim(strcode) <> Trim(HFleadtypecd.Value) And strcode <> "" Then
                Return True
            Else
                Return False
                Exit Function
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim obj As New ClsLeadTypeMaster, objDas As New DBAccess
            obj.leadtypecd = HFleadtypecd.Value
            obj.leadtypename = objDas.FilteredString(txtleadtypename.Text)

            If HFleadtypecd.Value = "" Then
                obj.leadtypecd = ClsLeadTypeMaster.GetDocCode
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtleadtypename.Text & "' Already Exist"
                    ErrorMsg0.Visible = True
                    Exit Try
                Else
                    ClsLeadTypeMaster.Update_Data(obj, "ADD")
                End If
            Else
                If Check_Duplicate() = True Then
                    ErrorMsg0.Text = "'" & txtleadtypename.Text & "' Already Exist"
                    ...
                    Exit Try
                Else
                    ClsLeadTypeMaster.Update_Data(obj, "EDIT")
                End If
            End If
            'FillData()
            CreateGridView()
    ...
    Protected Sub btnDelete_Click(sender As Object, e As System.EventArgs) Handles btnDelete.Click
        Try
            If HFleadtypecd.Value <> "" Then
                Dim str As String
                str = ClsLeadTypeMaster.Check_Integrity(HFleadtypecd.Value).ToString.Trim
                If str <> "" Then
                    ...
                    Dim myscript As String = "alert('Lead Type Being Used In " & Trim(str) & "');"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)

                    ErrorMsg0.Visible = True
                    ErrorMsg0.Text = ""
                    ErrorMsg0.Text = "Lead Type Being Used In " & Trim(str)
                    str = ""
                    Exit Sub
                End If
