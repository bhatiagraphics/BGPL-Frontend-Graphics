...
Partial Class CompletedJobsforQC
    ...
    Private Sub CreateGridView()
        Completed_Jobs_for_QC_Check.SettingsBehavior.AllowFocusedRow = True
        Completed_Jobs_for_QC_Check.DataSource = CreateData()
        Completed_Jobs_for_QC_Check.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Completed_Jobs_QC_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "','" & Trim(ddlprinttype.SelectedItem.Value) & "'"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Completed_Jobs_for_QC_Check.CustomCallback
        Completed_Jobs_for_QC_Check.Columns.Clear()
        Completed_Jobs_for_QC_Check.AutoGenerateColumns = False
        Completed_Jobs_for_QC_Check.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Completed_Jobs_for_QC_Check.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Completed_Jobs_for_QC_Check.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Completed_Jobs_for_QC_Check.Columns
