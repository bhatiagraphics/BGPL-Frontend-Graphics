...
Partial Class SummaryCompletedJobsApprovalformGraphics
    ...
    Private Sub CreateGridView()
        Completed_Jobs_Approval_form_Graphics_Summary.SettingsBehavior.AllowFocusedRow = True
        Completed_Jobs_Approval_form_Graphics_Summary.DataSource = CreateData()
        Completed_Jobs_Approval_form_Graphics_Summary.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Summary_Completed_Jobs_ApprovalformGraphics_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "',''"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Completed_Jobs_Approval_form_Graphics_Summary.CustomCallback
        Completed_Jobs_Approval_form_Graphics_Summary.Columns.Clear()
        Completed_Jobs_Approval_form_Graphics_Summary.AutoGenerateColumns = False
        Completed_Jobs_Approval_form_Graphics_Summary.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Completed_Jobs_Approval_form_Graphics_Summary.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Completed_Jobs_Approval_form_Graphics_Summary.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Completed_Jobs_Approval_form_Graphics_Summary.Columns
    ...
    Protected Sub btnLedger_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If Trim(roomName) <> "" Then
            Response.Redirect("CompletedJobsApprovalformGraphics.aspx?assignedto=" + (Trim(roomName)) + "&FromPeriod=" + Trim(txtfromdt.Text) + "&ToPeriod=" + Trim(txttodt.Text) + "&empcd=" + Trim(Session.Item("UserID")))
        End If
    End Sub
End Class
