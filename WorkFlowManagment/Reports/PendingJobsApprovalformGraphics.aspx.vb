...
Partial Class PendingJobsApprovalformGraphics
    ...
    Private Sub CreateGridView()
        Pending_Jobs_Approval_form_Graphics.SettingsBehavior.AllowFocusedRow = True
        Pending_Jobs_Approval_form_Graphics.DataSource = CreateData()
        Pending_Jobs_Approval_form_Graphics.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Pending_Jobs_Sentto_Graphic_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "','" & Trim(ddlprinttype.SelectedItem.Value) & "','" & Trim(ddlassignedto.SelectedItem.Value) & "'"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Pending_Jobs_Approval_form_Graphics.CustomCallback
        Pending_Jobs_Approval_form_Graphics.Columns.Clear()
        Pending_Jobs_Approval_form_Graphics.AutoGenerateColumns = False
        Pending_Jobs_Approval_form_Graphics.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Pending_Jobs_Approval_form_Graphics.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Pending_Jobs_Approval_form_Graphics.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Pending_Jobs_Approval_form_Graphics.Columns
