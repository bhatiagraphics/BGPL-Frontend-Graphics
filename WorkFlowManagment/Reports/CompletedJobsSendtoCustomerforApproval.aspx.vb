...
Partial Class CompletedJobsSendtoCustomerforApproval
    ...
    Private Sub CreateGridView()
        Completed_Jobs_Send_to_Customer_for_Approval.SettingsBehavior.AllowFocusedRow = True
        Completed_Jobs_Send_to_Customer_for_Approval.DataSource = CreateData()
        Completed_Jobs_Send_to_Customer_for_Approval.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Completed_Jobs_Sentto_CustomerforApproval_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "','" & Trim(ddlprinttype.SelectedItem.Value) & "'"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Completed_Jobs_Send_to_Customer_for_Approval.CustomCallback
        Completed_Jobs_Send_to_Customer_for_Approval.Columns.Clear()
        Completed_Jobs_Send_to_Customer_for_Approval.AutoGenerateColumns = False
        Completed_Jobs_Send_to_Customer_for_Approval.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Completed_Jobs_Send_to_Customer_for_Approval.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Completed_Jobs_Send_to_Customer_for_Approval.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Completed_Jobs_Send_to_Customer_for_Approval.Columns
