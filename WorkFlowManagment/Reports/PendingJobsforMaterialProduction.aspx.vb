...
Partial Class PendingJobsforMaterialProduction
    ...
    Private Sub CreateGridView()
        Pending_Jobs_for_Material_Production.SettingsBehavior.AllowFocusedRow = True
        Pending_Jobs_for_Material_Production.DataSource = CreateData()
        Pending_Jobs_for_Material_Production.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Pending_Jobs_Material_Production_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "','" & Trim(ddlprinttype.SelectedItem.Value) & "'"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Pending_Jobs_for_Material_Production.CustomCallback
        Pending_Jobs_for_Material_Production.Columns.Clear()
        Pending_Jobs_for_Material_Production.AutoGenerateColumns = False
        Pending_Jobs_for_Material_Production.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Pending_Jobs_for_Material_Production.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Pending_Jobs_for_Material_Production.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Pending_Jobs_for_Material_Production.Columns
