...
Partial Class PendingJobsForRepress
    ...
    Private Sub CreateGridView()
        Pending_Jobs_For_Repress.SettingsBehavior.AllowFocusedRow = True
        Pending_Jobs_For_Repress.DataSource = CreateData()
        Pending_Jobs_For_Repress.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Pending_Jobs_Sentto_Prepress_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "','" & Trim(ddlprinttype.SelectedItem.Value) & "','" & Trim(ddlassignedto.SelectedItem.Value) & "'"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Pending_Jobs_For_Repress.CustomCallback
        Pending_Jobs_For_Repress.Columns.Clear()
        Pending_Jobs_For_Repress.AutoGenerateColumns = False
        Pending_Jobs_For_Repress.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Pending_Jobs_For_Repress.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Pending_Jobs_For_Repress.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Pending_Jobs_For_Repress.Columns
