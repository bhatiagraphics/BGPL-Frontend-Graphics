...
Partial Class CompletedJobsforBilling
    ...
    Private Sub CreateGridView()
        Completed_Jobs_for_Billing.SettingsBehavior.AllowFocusedRow = True
        Completed_Jobs_for_Billing.DataSource = CreateData()
        Completed_Jobs_for_Billing.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Completed_Jobs_Billing_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "','" & Trim(ddlprinttype.SelectedItem.Value) & "'"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Completed_Jobs_for_Billing.CustomCallback
        Completed_Jobs_for_Billing.Columns.Clear()
        Completed_Jobs_for_Billing.AutoGenerateColumns = False
        Completed_Jobs_for_Billing.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Completed_Jobs_for_Billing.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Completed_Jobs_for_Billing.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Completed_Jobs_for_Billing.Columns
