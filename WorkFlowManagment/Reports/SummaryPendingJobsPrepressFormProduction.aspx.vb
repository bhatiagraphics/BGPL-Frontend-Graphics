...
Partial Class SummaryPendingJobsPrepressFormProduction
    ...
    Private Sub CreateGridView()
        Pending_Jobs_Prepress_Form_Production_Summary.SettingsBehavior.AllowFocusedRow = True
        Pending_Jobs_Prepress_Form_Production_Summary.DataSource = CreateData()
        Pending_Jobs_Prepress_Form_Production_Summary.DataBind()
    End Sub

    Private Function CreateData() As DataTable
        Dim dt As New DataTable()
        Using con As New SqlConnection(thisConnectionString)
            Using cmd As New SqlCommand("Summary_Pending_Jobs_Sentto_Prepress_sp", con)
                ...
                cmd.Parameters.AddWithValue("@printcd", "") ' Assuming an empty string for the unused parameter
    ...
    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Pending_Jobs_Prepress_Form_Production_Summary.CustomCallback
        Pending_Jobs_Prepress_Form_Production_Summary.Columns.Clear()
        Pending_Jobs_Prepress_Form_Production_Summary.AutoGenerateColumns = False
        Pending_Jobs_Prepress_Form_Production_Summary.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Pending_Jobs_Prepress_Form_Production_Summary.DataBinding
    ...
    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Pending_Jobs_Prepress_Form_Production_Summary.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Pending_Jobs_Prepress_Form_Production_Summary.Columns
    ...
    Protected Sub btnLedger_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If Trim(roomName) <> "" Then
            Response.Redirect("PendingJobsForRepress.aspx?assignedto=" + (Trim(roomName)) + "&FromPeriod=" + Trim(txtfromdt.Text) + "&ToPeriod=" + Trim(txttodt.Text) + "&empcd=" + Trim(Session.Item("UserID")))
        End If
    End Sub
End Class
