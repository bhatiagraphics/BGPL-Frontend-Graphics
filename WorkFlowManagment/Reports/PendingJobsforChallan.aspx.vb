...
Partial Class PendingJobsforChallan
    ...
    Private Sub CreateGridView()
        Pending_Jobs_for_Challan.SettingsBehavior.AllowFocusedRow = True
        Pending_Jobs_for_Challan.DataSource = CreateData()
        Pending_Jobs_for_Challan.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Pending_Jobs_Challan_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "','" & Trim(ddlprinttype.SelectedItem.Value) & "'"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Pending_Jobs_for_Challan.CustomCallback
        Pending_Jobs_for_Challan.Columns.Clear()
        Pending_Jobs_for_Challan.AutoGenerateColumns = False
        Pending_Jobs_for_Challan.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Pending_Jobs_for_Challan.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Pending_Jobs_for_Challan.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Pending_Jobs_for_Challan.Columns
