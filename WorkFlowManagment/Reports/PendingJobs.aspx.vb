...
Partial Class PendingJobs
    ...
    Private Sub CreateGridView()
        Pending_jobs.SettingsBehavior.AllowFocusedRow = True
        Pending_jobs.DataSource = CreateData()
        Pending_jobs.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Pending_Jobs_ApprovalformGraphics_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "','" & Trim(ddlprinttype.SelectedItem.Value) & "'"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Pending_jobs.CustomCallback
        Pending_jobs.Columns.Clear()
        Pending_jobs.AutoGenerateColumns = False
        Pending_jobs.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Pending_jobs.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Pending_jobs.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Pending_jobs.Columns
