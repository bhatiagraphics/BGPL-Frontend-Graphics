...
Partial Class PendingJobsforBilling
    ...
    Private Sub CreateGridView()
        Pending_Jobs_for_Billing.SettingsBehavior.AllowFocusedRow = True
        Pending_Jobs_for_Billing.DataSource = CreateData()
        Pending_Jobs_for_Billing.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Pending_Jobs_Billing_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "','" & Trim(ddlprinttype.SelectedItem.Value) & "'"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Pending_Jobs_for_Billing.CustomCallback
        Pending_Jobs_for_Billing.Columns.Clear()
        Pending_Jobs_for_Billing.AutoGenerateColumns = False
        Pending_Jobs_for_Billing.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Pending_Jobs_for_Billing.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Pending_Jobs_for_Billing.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Pending_Jobs_for_Billing.Columns
