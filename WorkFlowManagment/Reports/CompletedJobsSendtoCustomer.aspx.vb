...
Partial Class CompletedJobsSendtoCustomer
    ...
    Private Sub CreateGridView()
        Completed_Jobs_Send_to_Customer.SettingsBehavior.AllowFocusedRow = True
        Completed_Jobs_Send_to_Customer.DataSource = CreateData()
        Completed_Jobs_Send_to_Customer.DataBind()
    End Sub

    Private Function CreateData() As SqlDataSource
        Dim selectCmnd As String = "Exec Completed_Jobs_Sento_Customer_sp '" & (txtfromdt.Text) & "', '" & (txttodt.Text) & "','" & Trim(Session.Item("UserID")) & "','" & Trim(ddlprinttype.SelectedItem.Value) & "'"
        Return New SqlDataSource(thisConnectionString, selectCmnd)
    End Function

    Protected Sub ASPxGridView1_CustomCallback(ByVal sender As Object, ByVal e As ASPxGridViewCustomCallbackEventArgs) Handles Completed_Jobs_Send_to_Customer.CustomCallback
        Completed_Jobs_Send_to_Customer.Columns.Clear()
        Completed_Jobs_Send_to_Customer.AutoGenerateColumns = False
        Completed_Jobs_Send_to_Customer.DataBind()
    End Sub

    Protected Sub ASPxGridView1_DataBinding(ByVal sender As Object, ByVal e As EventArgs) Handles Completed_Jobs_Send_to_Customer.DataBinding
        TryCast(sender, ASPxGridView).DataSource = CreateData()
    End Sub

    Protected Sub ASPxGridView1_HeaderFilter(sender As Object, e As DevExpress.Web.ASPxGridViewHeaderFilterEventArgs) Handles Completed_Jobs_Send_to_Customer.HeaderFilterFillItems
    ...
    Private Sub GridChkbox()
        ...
        For Each column As GridViewDataColumn In Completed_Jobs_Send_to_Customer.Columns
