...
Partial Class Dispatch
    ...
    Private Sub Fillcombo()
        Try
            ...
            dt3 = objDas.GetDataTable(sSQL)
            dt3.Rows.InsertAt(dt3.NewRow, 0)
            ddlassignedto_dispatch.DataSource = dt3
            ddlassignedto_dispatch.DataTextField = "u_name"
            ddlassignedto_dispatch.DataValueField = "u_id"
            ddlassignedto_dispatch.DataBind()
    ...
    Protected Sub New_Docno()
        Try
            j = clsDispatch.Fetch_Paramno("")
            txtjobid.Text = clsDispatch.Generate_Docno(j, "", "").ToString.Trim
        Catch ex As Exception
            ErrorMSG.Visible = True
            ErrorMSG.Text = ""
            ErrorMSG.Text = ex.Message
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                Else
                    ...
                    If Check_AccountAllowed(Trim(Session.Item("UserID"))) = "Allowed" Then
                        btnSendForBilling.Enabled = True
                    Else
                        btnSendForBilling.Enabled = False
                    End If
    ...
    Private Sub DisplayData(ByVal geno As String)
        ...
        con = clsDispatch.GetConnection()
        com = clsDispatch.GetCommand
        com.Connection = con
        com.CommandType = CommandType.Text
        Try
            Dim obj As clsDispatch = clsDispatch.FillData(geno, "B")
            If Not IsNothing(obj) Then
                ...
                ddlassignedto_dispatch.SelectedItem.Selected = False
                ddlassignedto_dispatch.Items.FindByValue(Trim(obj.assignedto_dispatch)).Selected = True
                ...
                txtdocketno.Text = Trim(obj.docketno)
                txtdispatch_comments.Text = Trim(obj.dispatch_comments)
                ...
                If obj.sendfor_billing_flag = "1" Then
                    btnSendForBilling.Visible = False
                End If
    ...
    Private Sub Save_Data_Add()
        ...
        Try
            Dim obj As New clsDispatch, objDas As New DBAccess
            ...
            obj.assignedto_dispatch = ddlassignedto_dispatch.SelectedValue
            ...
            clsDispatch.Update_Data_Add(obj, j, mode, "")
    ...
    Private Sub Save_Data_Edit()
        ...
        Try
            ...
            obj.transcd = ddltranscd.SelectedValue
            obj.docketno = txtdocketno.Text
            obj.plate_processed_date = txtplate_processed_date.Text
            obj.dispatch_comments = txtdispatch_comments.Text
            ...
            clsDispatch.Update_Data_Edit(obj, j, mode, "")
            If ErrorMSG.Text = "" Then
                Response.Write("<script>alert('Details Saved Succesfully'); ")
                Response.Write("window.location='DispatchSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
            End If
        Catch ex As Exception
            HFsave_value.Value = ""
            ErrorMSG.Visible = True
    ...
    Private Function Check_AccountAllowed(ByVal u_id As String) As String
        Check_AccountAllowed = ""
        Dim sSQL1 As String, dt1 As DataTable, objDas As New DBAccess
        sSQL1 = "select allowbilling from users where u_id = '" & Trim(u_id) & "'"
        dt1 = objDas.GetDataTable(sSQL1)
        If dt1.Rows.Count > 0 Then
            If Trim(dt1.Rows(0)("allowbilling").ToString) = "1" Then
                Check_AccountAllowed = "Allowed"
            Else
                Check_AccountAllowed = ""
            End If
        Else
    ...
    'Protected Sub btnSendToGrphics_Click(sender As Object, e As System.EventArgs) Handles btnSendForQC.Click
    '    Try
    '        Call Save_Data_Edit()
    '        Dim sSQL As String
    '        Dim dt1, dt2, dt3 As DataTable
    '        Dim objDas As New DBAccess
    '        sSQL = "update jobentry set sendfor_billing_flag='1',sendfor_billing_uid='" & Trim(Session.Item("UserID")) & "',sendfor_billing_udate=GetDate(),senfor_billing_comments='" & txtsenfor_billing_comments.Text & "' where jobid='" & txtjobid.Text & "'"
    '        dt1 = objDas.GetDataTable(sSQL)
    '        Response.Write("<script>alert('Send To Billing Succesfully'); ")
    '        Response.Write("window.location='QCSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
    ...
    Protected Sub btnSendForBilling_Click(sender As Object, e As System.EventArgs) Handles btnSendForBilling.Click
        Try
            Dim sSQL As String
            Dim dt1, dt2, dt3 As DataTable
            Dim objDas As New DBAccess
            'sSQL = "update jobentry set transcd='" & ddltranscd.SelectedValue & "',docketno='" & txtdocketno.Text & "',dispatch_close_flag='1',dispatch_comments='" & txtdispatch_comments.Text & "',distpatch_uid='" & Trim(Session.Item("UserID")) & "',dispatch_udate=GetDate() where jobid='" & txtjobid.Text & "'"
            sSQL = "update jobentry set transcd='" & ddltranscd.SelectedValue & "',docketno='" & txtdocketno.Text & "', sendfor_billing_flag='1',sendfor_billing_uid='" & Trim(Session.Item("UserID")) & "',sendfor_billing_udate=GetDate(),senfor_billing_comments='" & txtdispatch_comments.Text & "',assignedto_dispatch='" & Trim(ddlassignedto_dispatch.SelectedValue) & "',assignedto_billing='" & Trim(ddlassignedto_dispatch.SelectedValue) & "'  where jobid='" & txtjobid.Text & "'"

            dt1 = objDas.GetDataTable(sSQL)
            Response.Write("<script>alert('Job to Billing Succesfully'); ")
            Response.Write("window.location='DispatchSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
    ...
    Protected Sub btnClose_Click(sender As Object, e As System.EventArgs) Handles btnclose.Click
        Response.Redirect("DispatchSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "")
    End Sub
