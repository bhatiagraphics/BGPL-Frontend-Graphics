...
Partial Class ChallanForm
    ...
    Protected Sub New_Docno()
        Try
            j = clsChallanForm.Fetch_Paramno("")
            txtjobid.Text = clsChallanForm.Generate_Docno(j, "", "").ToString.Trim
        Catch ex As Exception
            ErrorMSG.Visible = True
            ErrorMSG.Text = ""
            ErrorMSG.Text = ex.Message
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not IsPostBack Then
                ...
                btnResetChallanInTally.Attributes.Add("onclick", "return confirm('Do you want to Reset Challan?');")
                btnCreateInTally.Attributes.Add("onclick", "return confirm('Do you want to Create Challan in Tally?');")
    ...
    Private Sub DisplayData(ByVal geno As String)
        ...
        con = clsChallanForm.GetConnection()
        com = clsChallanForm.GetCommand
        com.Connection = con
        com.CommandType = CommandType.Text
        Try
            Dim obj As clsChallanForm = clsChallanForm.FillData(geno, "B")
    ...
    Private Sub Save_Data_Add()
        ...
        Try
            Dim obj As New clsChallanForm, objDas As New DBAccess
            ...
            clsChallanForm.Update_Data_Add(obj, j, mode, "")
    ...
    Private Sub Save_Data_Edit()
        ...
        Try
            ...
            clsChallanForm.Update_Data_Edit(obj, j, mode, "")
    ...
    Private Sub save_edit()
        ...
        If ErrorMSG.Text = "" Then
            ...
            Response.Write("window.location='ChallanFormSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
        End If
    End Sub
    ...
    Protected Sub btnClose_Click(sender As Object, e As System.EventArgs) Handles btnclose.Click
        Response.Redirect("ChallanFormSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "")
    End Sub
    ...
    Private Sub CreateDeliveryNoteInTally(ByVal JobId As String, ByVal mode As String)
        Try
            Dim ConnStr As String = ConfigurationManager.ConnectionStrings("ConStr").ConnectionString
            Dim cls As New ClsTallyIntegration()

            If Not cls.IsCompanyLoaded() Then
                ErrorMSG.Visible = True
                ErrorMSG.Text = cls.TallyCompanyName + " not found!! \n Please check if Tally is running and Company is selected!! "
                Exit Sub
            End If

            If mode = "Reset" Then
                If cls.VoucherExistInTally(Trim(JobId)) = True Then
                    Dim strqry As String = "Delete from TallyVoucherMasterIDMapping where jobid='" & Trim(JobId) & "'"
                    cls.ExecuteSQL(strqry, ConnStr)

                    strqry = "Update JobEntry set chlno='',chldt= '' where jobid='" & Trim(JobId) & "'"
                    If cls.ExecuteSQL(strqry, ConnStr) > 0 Then
                        txtchlno.Text = ""
                        'txtchldt.Text = DateTime.Now.ToString("dd/MM/yyyy")
                        txtchldt.Text = ""
                    End If
                    ErrorMSG.Visible = True
                    ErrorMSG.Text = "Tally Challan reset successfully!! "
                Else
                    ErrorMSG.Visible = True
                    ErrorMSG.Text = "Tally Challan not found!! "
                End If
                Exit Sub
            End If
            ...
            If mode = "Delete" Then
                If cls.VoucherExistInTally(Trim(JobId)) = True Then
                    Dim resp = cls.CreateDeliveryNoteInTally(Trim(JobId), "Delete")
                    ErrorMSG.Visible = True
                    ErrorMSG.Text = resp
                    Exit Sub
                End If
            ...
            If mode <> "Delete" Then
                If cls.VoucherExistInTally(JobId) = True Then
                    'Dim resp = cls.CreateDeliveryNoteInTally(Trim(JobId), "Alter")

                    'If cls.TallyCreated Then
                    '    Dim parsedDate As DateTime = DateTime.ParseExact(cls.TallyVoucherDate, "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture)
                    '    Dim formattedDateString As String = parsedDate.ToString("yyyy-MM-dd")
                    '    Dim strqry As String = "Update JobEntry set chlno='" & Trim(cls.TallyVoucherNo) & "',chldt= '" & formattedDateString & "' where jobid='" & Trim(JobId) & "'"
                    '    If cls.ExecuteSQL(strqry, ConnStr) > 0 Then
                    '        txtchlno.Text = Trim(cls.TallyVoucherNo)
                    '        txtchldt.Text = parsedDate.ToString("dd/MM/yyyy")
                    '    End If
                    'End If

                    'If cls.TallyEdited Then
                    '    Dim parsedDate As DateTime = DateTime.ParseExact(cls.TallyVoucherDate, "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture)
                    '    Dim formattedDateString As String = parsedDate.ToString("yyyy-MM-dd")
                    '    Dim strqry As String = "Update JobEntry set chlno='" & Trim(cls.TallyVoucherNo) & "',chldt= '" & formattedDateString & "' where jobid='" & Trim(JobId) & "'"
                    '    If cls.ExecuteSQL(strqry, ConnStr) > 0 Then
                    ...
                    Dim resp = "Challan already exists!"
                    ErrorMSG.Visible = True
                    ErrorMSG.Text = resp
                Else
                    Dim resp = cls.CreateDeliveryNoteInTally(Trim(JobId), "Create")
                    If cls.TallyCreated Then
                        Dim parsedDate As DateTime = DateTime.ParseExact(cls.TallyVoucherDate, "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture)
                        Dim formattedDateString As String = parsedDate.ToString("yyyy-MM-dd")
                        Dim strqry As String = "Update JobEntry set chlno='" & Trim(cls.TallyVoucherNo) & "',chldt= '" & formattedDateString & "' where jobid='" & Trim(JobId) & "'"
        ...
        End Try
    End Sub
    Protected Sub btnCreateInTally_Click(sender As Object, e As EventArgs) Handles btnCreateInTally.Click
        CreateDeliveryNoteInTally(txtjobid.Text, "Create")
    End Sub

    Protected Sub btnResetChallanInTally_Click(sender As Object, e As EventArgs) Handles btnResetChallanInTally.Click
        'ScriptManager.RegisterStartupScript(Page, GetType(Page), "ChallanResetConfirm", "return confirm('Do you want to Reset Challan?');", True)
        'Dim script As String = "if(!confirm('Do you want to Reset Challan?')) return false;"
        'ScriptManager.RegisterClientScriptBlock(btnResetChallanInTally, btnResetChallanInTally.GetType(), "ChallanResetConfirm", Script, True)
        CreateDeliveryNoteInTally(txtjobid.Text, "Reset")
