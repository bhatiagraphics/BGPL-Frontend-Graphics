...
Partial Class BillingForm
    ...
    Private Sub Fillcombo()
        Try
            ...
            dt3 = objDas.GetDataTable(sSQL)
            dt3.Rows.InsertAt(dt3.NewRow, 0)
            ddlcuscodeshipto.DataSource = dt3
            ddlcuscodeshipto.DataTextField = "cusname"
            ddlcuscodeshipto.DataValueField = "cuscode"
            ddlcuscodeshipto.DataBind()
    ...
    Protected Sub New_Docno()
        Try
            j = clsBillingForm.Fetch_Paramno("")
            txtjobid.Text = clsBillingForm.Generate_Docno(j, "", "").ToString.Trim
        Catch ex As Exception
            ErrorMSG.Visible = True
            ErrorMSG.Text = ""
            ErrorMSG.Text = ex.Message
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not IsPostBack Then
                ...
                '    txtfromdt.Text = Trim(Request.QueryString("FromPeriod"))
                '    txttodt.Text = Trim(Request.QueryString("ToPeriod"))
                '    ddlassignedto.SelectedValue = Trim(Request.QueryString("assignedto"))
                '    CreateGridView()
                '    txtfromdt.Enabled = False
                '    txtfromdt_CalendarExtender.Enabled = False
                '    txttodt.Enabled = False
                '    txttodt_CalendarExtender.Enabled = False
                '    ddlassignedto.Enabled = False
                '    ddlprinttype.Enabled = False
                ...
                Else
                    ...
                    If Check_AccountAllowed(Trim(Session.Item("UserID"))) = "Allowed" Then
                        btnbillingdonejobclose.Enabled = True
                    Else
                        btnbillingdonejobclose.Enabled = False
                    End If
                    txtjobcreatedt.Focus()
                End If
    ...
    Private Sub DisplayData(ByVal geno As String)
        ...
        con = clsBillingForm.GetConnection()
        com = clsBillingForm.GetCommand
        com.Connection = con
        com.CommandType = CommandType.Text
        Try
            Dim obj As clsBillingForm = clsBillingForm.FillData(geno, "B")
            If Not IsNothing(obj) Then
                ...
                ddlcuscodeshipto.SelectedItem.Selected = False
                ddlcuscodeshipto.Items.FindByValue(Trim(obj.cuscodeshipto)).Selected = True
                ...
                If Trim(obj.dispatch_close_flag) = "1" Then
                    btnbillingdonejobclose.Visible = False
                    btnSave.Visible = False
                    btnSave.Visible = False
                End If
    ...
    Private Sub Save_Data_Add()
        ...
        Try
            Dim obj As New clsBillingForm, objDas As New DBAccess
            ...
            obj.cuscodeshipto = ddlcuscodeshipto.SelectedValue
            ...
            clsBillingForm.Update_Data_Add(obj, j, mode, "")
    ...
    Private Sub Save_Data_Edit()
        ...
        Try
            ...
            clsBillingForm.Update_Data_Edit(obj, j, mode, "")
    ...
    Private Sub save_edit()
        ...
        If ErrorMSG.Text = "" Then
            ...
            Response.Write("window.location='BillingFormSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
        End If
    End Sub
    ...
    Protected Sub btnbillingdonejobclose_Click(sender As Object, e As System.EventArgs) Handles btnbillingdonejobclose.Click
        If Check_AccountAllowed(Trim(Session.Item("UserID"))) = "Allowed" Then
        Else
            Dim myscript As String = "alert('No Access is Given. Please contact the administrator.');"
        ...
        Try
            ...
            Dim dt1, dt2, dt3 As DataTable
            Dim objDas As New DBAccess
            sSQL = "update jobentry set dispatch_close_flag='1',dispatch_comments='" & txtsendfor_dispatch_comments.Text & "',distpatch_uid='" & Trim(Session.Item("UserID")) & "',dispatch_udate=GetDate(),assignedto_billing='" & Trim(ddlassignedto.SelectedValue) & "'  where jobid='" & txtjobid.Text & "'"
            dt1 = objDas.GetDataTable(sSQL)
            Response.Write("<script>alert('Billing Done & Job Closed'); ")
            Response.Write("window.location='BillingFormSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
    ...
    Protected Sub btnClose_Click(sender As Object, e As System.EventArgs) Handles btnclose.Click
        Response.Redirect("BillingFormSearch.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "")
    End Sub
    ...
    Private Function Check_AccountAllowed(ByVal u_id As String) As String
        ...
        Dim sSQL1 As String, dt1 As DataTable, objDas As New DBAccess
        sSQL1 = "select allowbilldone_close from users where u_id = '" & Trim(u_id) & "'"
        dt1 = objDas.GetDataTable(sSQL1)
        If dt1.Rows.Count > 0 Then
            If Trim(dt1.Rows(0)("allowbilldone_close").ToString) = "1" Then
