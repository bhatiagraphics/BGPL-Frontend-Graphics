...
Partial Class OpenJobGrpahics_Hold
    ...
    Private Sub DisplayData(ByVal geno As String)
        ...
        Try
            ...
            If Not IsNothing(obj) Then
                ...
                ddlassignedto.Items.FindByValue(Trim(obj.assignedto_appformgraphic)).Selected = True
    ...
    Private Sub Save_Data_Add()
        ...
        Try
            ...
            obj.assignedto_appformgraphic = ddlassignedto.SelectedValue
    ...
    Private Sub save_edit()
        ...
        If ErrorMSG.Text = "" Then
            ...
            Response.Write("window.location='OpenJobGrpahicsSearch_Hold.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
        End If
    End Sub


    Protected Sub btnUnHold_Click(sender As Object, e As System.EventArgs) Handles btnUnHold.Click
        Try
            Call Save_Data_Edit()
            Dim sSQL As String
            Dim dt1, dt2, dt3 As DataTable
            Dim objDas As New DBAccess
            sSQL = "update jobentry set holdsendtographics_flag='',sendtographics_flag='1',sentographic_uid='" & Trim(Session.Item("UserID")) & "',sentographic_dt=GetDate()  where jobid='" & txtjobid.Text & "'"
            dt1 = objDas.GetDataTable(sSQL)
            Response.Write("<script>alert('Job is UnHold'); ")
            Response.Write("window.location='OpenJobGrpahicsSearch_Hold.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "'</script>")
    ...
    'Protected Sub btnSendToGrphics_Click(sender As Object, e As System.EventArgs) Handles btnSendToCustomer.Click
    '    Try
    '        Call Save_Data_Edit()
    '        Dim sSQL As String
    '        Dim dt1, dt2, dt3 As DataTable
    '        Dim objDas As New DBAccess
    '        sSQL = "update jobentry set sento_customer_flag='1',sento_customer_uid='" & Trim(Session.Item("UserID")) & "',sento_customer_date=GetDate() where jobid='" & txtjobid.Text & "'"
    '        dt1 = objDas.GetDataTable(sSQL)
    ...
    Protected Sub btnClose_Click(sender As Object, e As System.EventArgs) Handles btnClose.Click
        Response.Redirect("OpenJobGrpahicsSearch_Hold.aspx?MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "")
    End Sub
    ...
    '    Response.Redirect("OpenJobGrpahicsSearch_Hold.aspx?docno=" & Trim(newqtnno) & "&MMModuleid=" & Trim(Request.QueryString("MMModuleid")) & "&mainid=" & Trim(Request.QueryString("mainid")) & "&subid=" & Trim(Request.QueryString("subid")) & "")
