...
Partial Class MasterPage2EwayBill
    Inherits System.Web.UI.MasterPage

    Dim StrBuilderMenu As StringBuilder
    Dim StrBuilderNotification As StringBuilder
    ...
    Private Sub GetMenu()
        ...
        '    StrBuilderMenu.Append("<li class='treeview'>")

        '    StrBuilderMenu.Append("<a href='" + dt.Rows(i)("NavigateUrl").ToString + "'><i class='fa fa-share'></i> <span>" + dt.Rows(i)("module_name").ToString + "</span><span class='pull-right-container'><i class='fa fa-angle-left pull-right'></i></span></a>")

        '    Dim dt1 As DataTable = GetDataAll(Convert.ToInt16(dt.Rows(i)("module_id").ToString))

        '    If dt1.Rows.Count > 0 Then
        '        StrBuilderMenu.Append("<ul class='treeview-menu' style='display: none;'>")
        '        For j As Integer = 0 To dt1.Rows.Count - 1

        '            Dim dt2 As DataTable = GetDataAll(Convert.ToInt16(dt1.Rows(i)("module_id").ToString))
        '            If dt2.Rows.Count > 0 Then
        '                StrBuilderMenu.Append("<ul class='treeview-menu' style='display: none;'>")
        '                StrBuilderMenu.Append("<li class='treeview' style='height: auto;'>")
        '                StrBuilderMenu.Append("<a href='#'><i class='fa fa-circle-o'></i> " + dt1.Rows(0)("module_name").ToString + " <span class='pull-right-container'><i class='fa fa-angle-left pull-right'></i></span></a>")

        '                StrBuilderMenu.Append("<li class='treeview' style='height: none;'>")
        '                StrBuilderMenu.Append("<ul class='treeview-menu' style='display: none;'>")

        '                For K As Integer = 0 To dt2.Rows.Count - 1

        '                    StrBuilderMenu.Append(" <li><a href='" + +dt1.Rows(K)("NavigateUrl").ToString + +"'><i class='fa fa-circle-o' ></i> " + dt2.Rows(j)("module_name").ToString + "</a></li>")
        ...
        '                StrBuilderMenu.Append("<li><a href='" + dt1.Rows(j)("NavigateUrl").ToString + "'><i class='fa fa-circle-o'></i> " + dt1.Rows(j)("module_name").ToString + "</a></li>")
        '            End If
        '        Next
        '        StrBuilderMenu.Append("</ul>")
        '    End If
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            If Trim(dt.Rows(i)("module_name").ToString) = "CRM" Then
                StrBuilderMenu.Append("<a  target='_blank' class='" + SideIcon + " waves-effect waves-dark' href='" + url + "' aria-expanded='False'> <i class='" + MenuIcon + "'></i><span class='hide-menu'>" + Trim(dt.Rows(i)("module_name").ToString) + "</span></a>")
            Else
